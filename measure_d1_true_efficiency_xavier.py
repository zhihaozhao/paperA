...
Using real model definitions from src/models.py to ensure consistency with experiments
"""
...
try:
    ...
    # Debug: Check what models are supported
    import inspect
    source = inspect.getsource(build_model)
    print(f"📝 build_model function source preview:")
    lines = source.split('\n')
...
def main():
    ...
    # Initialize models using real model definitions
    # Based on actual build_model function, available models are:
    # enhanced (BiLSTM), lstm (LSTM32), tcn (TCN), txf (TinyTransformer)  
    model_names = ['enhanced', 'lstm', 'tcn']  # Skip txf for now due to potential implementation issues
    models = {}
    
    print(f"\n📋 Building Real Models from src/models.py:")
    for model_name in model_names:
        try:
            ...
            model = build_model(name=model_name, input_dim=args.F, num_classes=args.classes, logit_l2=0.05)
            models[model_name] = model
            param_count = count_parameters(model)
            print(f"  ✅ {model_name}: {param_count:,} ({param_count/1000:.1f}K) parameters")
    ...
    results = {
        ...
        'experiment_config': {
            ...
            'input_shape': input_shape,
            'description': 'D1 true parameter configuration using real models from src/models.py'
        },
        'results': {}
    }
    ...
    for model_name in model_names:
        if model_name in results['results'] and 'error' not in results['results'][model_name]:
            r = results['results'][model_name]
            edge_status = '✓' if r['edge_ready'] else '✗'
            print(f"{model_name:<15} {r['parameters']:<12,} {r['inference_mean_ms']:<15.2f} {r['memory_peak_mb']:<12.1f} {edge_status}")
