...
for %%m in (%MODELS%) do (
    for %%s in (%SEEDS%) do (
        ...
        python -m src.train_cross_domain --model %%m --seed %%s --epochs %EPOCHS% --protocol loro --benchmark_path "%BENCHMARK_PATH%" --files_per_activity %FILES_PER_ACTIVITY% --class_weight %CLASS_WEIGHT% %FOLDS_FLAG% %AMP_FLAG% --output_dir "%OUTPUT_DIR%" --out "!OUTPUT_FILE!"
