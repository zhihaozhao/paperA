#!/usr/bin/env python3
"""
Principal Component Analysis (PCA) Visualization - New Figure 6
...
def create_pca_biplot():
    ...
    for model in models:
        ...
        ax1.scatter(loro_data['PC1'], loro_data['PC2'], 
                   c=color, marker=marker, s=60, alpha=0.4,
    ...
    # Add arrows for feature loadings (if we had meaningful feature names)
    ...
    for i, (loading, name) in enumerate(zip(loadings, feature_names)):
        if np.linalg.norm(loading) > 0.3:  # Only show significant loadings
            ...
            ax1.text(loading[0]*3.2, loading[1]*3.2, name, 
    ...
    ax2.set_title('PCA Explained Variance', fontweight='bold', fontsize=10)
    ...
    ax3.set_title('Model Separation\nDistances', fontweight='bold', fontsize=10)
    ...
    ax4.set_ylabel('LOSO-LORO Distance (Lower = More Consistent)')
    ax4.set_title('Cross-Protocol Consistency Analysis', fontweight='bold')
    ...
    for model in models:
        ...
        ax5.scatter(model_data['PC1'], model_data['PC2'], model_data['PC3'],
                   c=color, marker=marker, s=30, alpha=0.6, label=model)
    
    ax5.set_xlabel('PC1')
    ax5.set_ylabel('PC2')  
    ax5.set_zlabel('PC3')
    ax5.set_title('3D Feature Space', fontweight='bold', fontsize=10)
...
def create_feature_importance_analysis():
    ...
    ax1.set_title('PCA Feature Loadings Matrix', fontweight='bold')
    ...
    ax2.set_yticklabels(pc1_contributions.index)
    ax2.set_xlabel('Absolute Loading Weight')
    ax2.set_title('Feature Contributions to Top 2 PCs', fontweight='bold')
    ax2.legend()
...
def export_pca_data():
    ...
    data_df[['Model', 'Protocol', 'PC1', 'PC2']].to_csv('figure6_pca_coordinates.csv', index=False)
    ...
    features_df.to_csv('figure6_feature_matrix.csv', index=False)
    ...
    pca_results_df.to_csv('figure6_pca_results.csv', index=False)
    ...
    variance_df.to_csv('figure6_explained_variance.csv', index=False)
    
    print("\n💾 PCA Data Export Complete:")
    print("• figure6_pca_coordinates.csv - PC1/PC2 coordinates for plotting")
    print("• figure6_feature_matrix.csv - Original feature matrix")
    print("• figure6_pca_results.csv - Full PCA transformation results")
    print("• figure6_explained_variance.csv - Variance explained by each component")

if __name__ == "__main__":
    print("🔍 Generating Advanced PCA Analysis - New Figure 6...")
    print("📊 Feature space clustering and dimensionality analysis")
    
    # Generate main PCA plot
    fig1, data_df, pca = create_pca_biplot()
    
    # Generate feature importance analysis
    fig2, loadings_df = create_feature_importance_analysis()
    
    # Save figures
    output_files = [
        ('figure6_pca_analysis.pdf', fig1),
        ('figure6_pca_analysis.png', fig1),
        ('figure6_feature_importance.pdf', fig2),
        ('figure6_feature_importance.png', fig2)
    ]
    ...
    print("🔍 New comprehensive feature space visualization")
    print("📊 Features: Biplot + loadings + clustering + protocol consistency")