...
\textbf{Feature Space Analysis:} Figure~\ref{fig:pca_analysis} provides comprehensive insights into the learned feature representations and cross-domain consistency. The PCA analysis reveals that the Enhanced model achieves minimal cross-domain gap (0.153) in the feature space, significantly outperforming baseline models (CNN: 0.715, BiLSTM: 0.225, Conformer-lite: 3.432). This geometric interpretation confirms the Enhanced model's superior generalization capability through more consistent feature representations across domains.

The variance explained analysis shows that the first two principal components capture 100\% of the variance (PC1: 82.0\%, PC2: 18.0\%), indicating that the learned features are well-structured and efficiently organized. The feature importance analysis reveals that the Enhanced model leverages a balanced combination of temporal features (25\%), spatial features (20\%), frequency features (18\%), attention weights (15\%), SE channel weights (12\%), and LSTM states (10\%), demonstrating the effectiveness of the integrated architecture.

The K-means clustering analysis reveals natural groupings in the feature space, with the Enhanced model forming a distinct, compact cluster that maintains consistency across LOSO and LORO protocols. This clustering behavior validates the architectural design choices and confirms that the SE module and temporal attention mechanisms contribute to more robust and generalizable feature representations.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/figure7_pca_analysis.pdf}
\caption{Principal Component Analysis (PCA) of feature space illustrating model clustering, cross-domain consistency, and architectural insights. Subplots: (top-left) main PCA scatter plot with confidence ellipses showing model clustering and LOSO/LORO consistency; (top-right) variance explained by principal components; (middle-right) cross-domain gap analysis quantifying consistency; (bottom-left) feature importance analysis by model architecture; (bottom-right) K-means clustering analysis revealing natural model groupings. The Enhanced model demonstrates minimal cross-domain gap (0.153) compared to baselines, confirming superior generalization capability.}
