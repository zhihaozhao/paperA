...
\caption{Experimental protocols overview: Synthetic Robustness Validation (540 configurations), \gls{cdae} (\gls{loso}/\gls{loro}), and \gls{stea} label-efficiency evaluation.}
\label{fig:protocols}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/figure3_enhanced_compatible.pdf}
\caption{Cross-domain generalization performance across \gls{loso} and \gls{loro}. The Enhanced model achieves identical 83.0±0.1\% macro F1 in both protocols with exceptional stability ($\text{CV}<0.2\%$).}
