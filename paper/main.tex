...
\usepackage{tcolorbox} % 加载 tcolorbox 宏包
\usepackage{enumitem}  % 处理枚举的宏包
...
%\textbf{Subject-Independent Methods:} 
...
%\textbf{Environment-Independent Methods:} 
...
%\textbf{Physics-Based Simulation:} 
...
%\textbf{Generative Model Approaches:} 
...
%\textbf{Transfer Learning Efficiency:} 
...
%\item \begin{tcolorbox}[colback=white,colframe=black,boxrule=0.5mm]
%\textbf{Activity Parameters:} Activity type, duration, movement patterns, number of subjects
%\end{tcolorbox}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig4_experimental_protocols.pdf}
...
%\textbf{Cause→Effect: Why CDAE and what it reveals.} \emph{Cause:} 
Real deployments face subject and environment shifts; training on one cohort or room often degrades in another. %\emph{Design:} 
CDAE isolates these axes via LOSO (subject shift) and LORO (environment shift), holding all else constant. %\emph{Effect:} 
...
\begin{figure*}[ht]
\centering
\includegraphics[width=\linewidth]{figures/fig5_cross_domain.pdf}%_cross-domain.pdf}
...
%\caption{Cross-domain generalization performance across LOSO and LORO. Subplots:(a)Performance metrics heatmap score for four models. (b)Model comparison radar chart showcase four aspects strengths by CNN and Enhanced models. (c)Metric Correlation Matrix of seven dimensions, the diagonal is default 1.0. (d)Composite score of over all model ranking bars. (e) Micro F1 of LOSO vc LORO across four models. The Enhanced model achieves identical 83.0±0.1\% macro F1 with exceptional stability ($\text{CV}<0.2\%$).}
%\caption{Cross-domain generalization performance across LOSO and LORO. Subplots: (a) main LOSO/LORO comparison with error bars; (top-right) cross-domain gap $|\text{LOSO}-\text{LORO}|$ (lower is better); (bottom-left) stability (CV, log-scale); (bottom-right) significance (−log$_{10}$(p)). The Enhanced model achieves identical 83.0±0.1\% macro F1 with exceptional stability ($\text{CV}<0.2\%$).}
\label{fig:cross_domain}
\end{figure*}

%\noindent\textit{Comparative context (CDAE):} 
...
%\textbf{Cross-Protocol Consistency:} 
...
%\textbf{Comprehensive Seven-Panel Feature Space Analysis:} 
...
%The main PCA biplot (a) revealing distinct clustering patterns where the Enhanced model demonstrates superior LOSO-LORO consistency through tightly overlapping protocol distributions, complemented by embedded legend for space efficiency; the cross-protocol consistency analysis (b) quantifying Enhanced model's minimal LOSO-LORO distance (0.08), significantly outperforming CNN (0.84), BiLSTM (0.23), and Conformer-lite (4.56); and the comprehensive PCA feature loadings matrix (d) providing heatmap visualization of how each feature dimension contributes to the first five principal components.
...
%\noindent\textit{Comparative context (STEA):} 
...
%\textbf{Label Efficiency Analysis:}
...
%\textbf{Transfer Method Comparison:} 
...
%\textbf{Cause→Effect synthesis.} \emph{Cause:} 
...
%We fix the synthetic data generator to the hard regime and progressively stress key nuisance parameters while training 5 random seeds per model. Specifically, we vary: class-overlap in $[0.6,0.8]$, noise standard deviation in $[0.4,0.6]$, and gain-drift standard deviation in $[0.4,0.6]$, while keeping sequence length $T{=}128$ and feature dimension $F{=}52$. Optimizer is Adam with learning rate $10^{-3}$ (cosine decay), batch size 768, early-stopping on macro F1. This emulates realistic, noisy deployments with increased ambiguity and acquisition noise.
%We repeat the best-performing PSTA settings for the top models (Enhanced, CNN) and run two additional GPU seeds to test repeatability in a resource-realistic setting. This captures whether gains are robust to implementation-level randomness (cudnn/tensor order) beyond the PSTA CPU-level seeds.
P%STA reveals stress robustness that enhanced and Conformer-lite sustain near-ceiling macro F1 despite harder overlap/noise/drift, whereas BiLSTM is more sensitive (high variance). 
%ESTA evidences stability that trends persist with minimal variance across fresh GPU runs, indicating training dynamics are repeatable and not artifacts of specific seeds or backends.
%Practitioners can dial difficulty via: (i) class-overlap ($0.6\rightarrow0.8$), (ii) noise std ($0.4\rightarrow0.6$), (iii) gain-drift std ($0.4\rightarrow0.6$). A recommended sweep is 3$\times$3$\times$3 grid over these ranges with 3--5 seeds per model. When compute-limited, keep $T{=}128,F{=}52$, lr $10^{-3}$ with cosine decay, and batch 512--768; adopt early-stopping on macro F1 with patience 10. For reproducibility, fix data-generation seeds separately from training seeds.
...
%Figure~\ref{fig:d5_d6_composite} presents a dual-panel analysis comparing the performance of models under the PSTA and ESTA protocols. Panel (a) illustrates the macro F1 scores for various models, including BILSTM, CNN, Conformer-lite, and the Enhanced model. The Enhanced model achieves a mean macro F1 score of 0.989 with a standard deviation of 0.016, indicating higher consistency and robustness compared to the CNN's score of 0.985 and the Conformer-lite's 0.992. Panel (b) further emphasizes model performance through the Brier score analysis, where the Enhanced model demonstrates a notably low Brier mean of 0.0021 compared to the other models. This showcases the Enhanced model's precision in generating reliable predictions with minimal uncertainty, reinforcing its efficacy in performance metrics across both protocols. The systematic layout of the figure facilitates direct comparison, enabling a comprehensive understanding of each model’s strengths and weaknesses.
...
%\textbf{Training process (the journey):} 
...
%Each run begins with a fresh initialization and a shuffled curriculum of the hard synthetic episodes. In the first epochs, models quickly latch onto gross temporal rhythms; as the schedule advances and overlap/noise rise, only architectures with richer channel attention (Enhanced) continue to carve separable manifolds. The cosine annealing gently lowers the step size so late-phase updates refine decision boundaries instead of overreacting to noisy bursts. Early-stopping is the guardrail: we track macro F1 on a held-out slice of the hard regime and stop when improvements stall, yielding comparable checkpoints across seeds.

%\textbf{Validation spec (what we guarantee):} 
...
%We validate on a fixed, unseen hard split drawn under the same parameter ranges but with disjoint seeds, to isolate generalization to \emph{conditions} (noise/overlap/drift) rather than memorization of episodes. Metrics include macro F1 (class-balance fairness) and Brier (probabilistic honesty). A run is accepted when: (i) the mean over seeds exceeds the model's own easy/medium baselines minus 1\%, (ii) the standard deviation remains within 2\% F1, and (iii) Brier tracks F1 (no inflated confidence). ESTA replays the acceptance with independent GPU seeds; passing both stages indicates the behavior is architectural, not incidental.
...
%\noindent\textit{Comparative context (PSTA/ESTA):} 
