...
\textbf{Scientific Seven-Panel Feature Space Analysis:} Figure~\ref{fig:pca_analysis} presents a scientifically organized seven-panel analysis arranged in three columns with optimal 4:3:3 width proportions for comprehensive feature space evaluation. The first column focuses on primary analysis with the main PCA biplot (top) revealing distinct clustering patterns where the Enhanced model demonstrates superior LOSO-LORO consistency through tightly overlapping protocol distributions, complemented by embedded legend for space efficiency. The cross-protocol consistency analysis (bottom, extended) quantifies this advantage, showing Enhanced achieves minimal LOSO-LORO distance (0.08), significantly outperforming CNN (0.84), BiLSTM (0.23), and Conformer-lite (4.56).

The second column provides statistical decomposition through three stacked analyses: variance explained analysis (top) showing that the first two principal components capture 28.3\% of total variance (PC1: 20.1\%, PC2: 8.2\%); model separation distance matrix (middle) quantifying inter-model distances in feature space; and 3D feature space visualization (bottom) confirming clustering patterns through three-dimensional projections where Enhanced model samples form coherent clusters across both evaluation protocols.

The third column delivers interpretability insights through PCA feature loadings matrix (top) providing comprehensive heatmap visualization of how each feature dimension contributes to the first five principal components, and feature contributions analysis (bottom, extended) specifically highlighting the relative importance to the top two principal components, revealing that Enhanced model's superior performance stems from balanced utilization of temporal (PC1: 0.45) and spatial features (PC2: 0.38).
...
\caption{Scientific seven-panel PCA analysis organized in three columns (4:3:3 width ratio) for comprehensive feature space evaluation. \textbf{Column 1 (Primary Analysis):} Top - Main PCA biplot with embedded legend, feature loading vectors, and confidence ellipses; Bottom (extended) - Cross-protocol consistency analysis revealing Enhanced model's minimal LOSO-LORO distance (0.08) versus baselines. \textbf{Column 2 (Statistical Decomposition):} Top - Explained variance analysis with cumulative curve; Middle - Inter-model separation distance heatmap; Bottom - 3D feature space clustering visualization. \textbf{Column 3 (Interpretability Analysis):} Top - PCA feature loadings matrix showing quantitative weights for all features across first five principal components; Bottom (extended) - Feature contributions analysis highlighting relative importance to top two principal components. The Enhanced model demonstrates superior cross-domain generalization through balanced temporal-spatial feature utilization, minimal protocol inconsistency, and optimal baseline separation.}
