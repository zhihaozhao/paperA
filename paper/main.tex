...
\textbf{Comprehensive Feature Space Analysis:} Figure~\ref{fig:pca_analysis} presents an extensive six-panel analysis of learned feature representations, providing multidimensional insights into model behavior and cross-domain consistency. The main PCA biplot (upper-left) reveals distinct clustering patterns where the Enhanced model demonstrates superior LOSO-LORO consistency through tightly overlapping protocol distributions, while baseline models exhibit significant protocol separation indicating poor cross-domain generalization.

The variance decomposition analysis (upper-right) shows that the first two principal components capture 28.3\% of total variance (PC1: 20.1\%, PC2: 8.2\%), with clear explained variance distribution across ten principal components. The model separation distance matrix (middle-right) quantifies inter-model distances in feature space, revealing that the Enhanced model maintains optimal separation from baselines while achieving internal consistency. Particularly notable is the cross-protocol consistency analysis (bottom-left), where Enhanced achieves minimal LOSO-LORO distance (0.08), significantly outperforming CNN (0.84), BiLSTM (0.23), and Conformer-lite (4.56).

The feature loading analysis with biplot vectors illustrates how key features—including Temporal_Pattern, Frequency_Response, Spatial_Correlation, and Attention_Weight—contribute to the principal components, providing interpretability into the Enhanced model's architectural advantages. The 3D feature space visualization (bottom-right) confirms these findings through three-dimensional clustering patterns, where Enhanced model samples form coherent clusters across both evaluation protocols.

The comprehensive analysis across all six visualization panels validates that the Enhanced model's architectural design—integrating SE attention and temporal mechanisms—produces more robust and generalizable feature representations. This multi-dimensional evidence strongly supports the Enhanced model's superior cross-domain performance observed in the main experimental results.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/figure7_pca_analysis.pdf}
\caption{Comprehensive Principal Component Analysis (PCA) featuring six complementary visualizations of feature space behavior and cross-domain consistency. (Upper-left) Main PCA biplot with feature loading vectors, confidence ellipses, and LOSO/LORO protocol comparison showing Enhanced model's superior consistency; (Upper-right) Explained variance analysis across all principal components with cumulative variance curve; (Middle-right) Inter-model separation distance heatmap quantifying feature space clustering; (Bottom-left) Cross-protocol consistency analysis revealing Enhanced model's minimal LOSO-LORO distance (0.08) versus baselines; (Bottom-right) 3D feature space visualization confirming clustering patterns across three principal components. The Enhanced model consistently demonstrates superior cross-domain generalization through tighter protocol alignment and optimal separation from baseline models.}
