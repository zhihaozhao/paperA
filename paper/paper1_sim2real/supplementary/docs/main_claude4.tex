...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{paper/enhanced/plots/attribution_examples.pdf}
\caption{Model Interpretability through Attribution Analysis: Visualization of attention patterns and feature importance maps generated by the Enhanced model. The attribution examples demonstrate that the model learns to focus on physically meaningful subcarrier patterns and temporal segments that align with known wireless propagation phenomena, providing interpretable insights into decision-making processes.}
\label{fig:attribution_examples}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{paper/enhanced/plots/d6_calibration_summary.pdf}
\caption{Enhanced Model Calibration Performance: Synthetic robustness evaluation (D6 protocol) showing macro F1 and Expected Calibration Error (ECE) across different stress conditions. The Enhanced architecture demonstrates superior calibration quality with ECE < 0.01 after temperature scaling, essential for trustworthy IoT deployment.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{paper/enhanced/plots/ablation_components.pdf}
\caption{Component-Level Ablation Study: Performance comparison across different architectural components of the Enhanced model. The ablation analysis demonstrates the synergistic effect of combining CNN feature extraction, SE channel attention, and temporal attention mechanisms, with each component contributing to the overall 83.0±0.1\% F1 cross-domain consistency.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{paper/zero/plots/transfer_compare.pdf}
\caption{Sim2Real Transfer Learning Comparison: Performance evolution from zero-shot baseline through linear probe to fine-tuning approaches. The transfer learning trajectory demonstrates effective knowledge transfer from synthetic pre-training, with rapid improvement using minimal real data. The Enhanced model achieves 82.1\% macro F1 at 20\% label ratio, representing 98.6\% of full-supervision performance.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{paper/enhanced/plots/ablation_noise_env_claude4.pdf}
\caption{Advanced Environmental Noise Ablation Analysis: Multi-perspective visualization showing macro F1 performance across varying class overlap and environmental burst conditions. Top row: Enhanced heatmaps with statistical indicators (mean±std). Middle row: Contour landscapes revealing performance gradients. Bottom row: 3D surface plots illustrating robustness characteristics. The Enhanced model demonstrates superior stability compared to CNN baseline, maintaining high performance (>0.85 F1) even under challenging stress conditions with combined class overlap (0.8) and environmental burst (0.2).}
\label{fig:noise_ablation_advanced}
\end{figure}
