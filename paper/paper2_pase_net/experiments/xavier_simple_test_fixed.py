...
Compatible with older PyTorch versions (removes batch_first argument)
"""
...
class SimplePASENet(nn.Module):
    """Simplified PASE-Net for testing - compatible with older PyTorch"""
    def __init__(self, input_dim=52, num_classes=6):
        ...
        # Temporal attention (simplified) - removed batch_first
        # For older PyTorch, MultiheadAttention expects (seq_len, batch, embed_dim)
...
class SimpleBiLSTM(nn.Module):
    """Simple BiLSTM baseline - compatible with older PyTorch"""
    def __init__(self, input_dim=52, hidden_dim=128, num_classes=6):
        super().__init__()
        # Remove batch_first argument for compatibility
        self.lstm = nn.LSTM(input_dim, hidden_dim, 2, bidirectional=True)
        self.fc = nn.Linear(hidden_dim * 2, num_classes)
    
    def forward(self, x):
        # x shape: (batch, time, features)
        # LSTM expects (seq_len, batch, features) when batch_first=False
        x = x.transpose(0, 1)  # (time, batch, features)
        out, _ = self.lstm(x)
        out = out[-1, :, :]  # Last timestep: (batch, features)
        return self.fc(out)
...
def main():
    ...
    # Generate LaTeX table if we have valid results
    if all('parameters_M' in r for r in results.values()):
        print(f"\n{'='*60}")
