...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{plots/fig4_cross_domain_enhanced.pdf}
\caption{Comprehensive Cross-Domain Performance Analysis: (a) LOSO performance comparison showing PASE-Net's exceptional stability (83.0±0.1\%) across subjects while Conformer exhibits convergence issues (40.3±34.5\%); (b) LORO performance in horizontal bar format highlighting consistent cross-room generalization; (c) Combined LOSO/LORO comparison revealing PASE-Net's unique domain consistency with zero performance gap; (d) Domain gap analysis (LOSO-LORO) demonstrating architectural advantages with PASE-Net showing minimal variation; (e) PASE-Net multi-metric radar chart illustrating balanced performance across robustness dimensions; (f) Statistical summary table with detailed metrics, confidence intervals, and performance gaps. The visualization confirms PASE-Net's superior cross-domain stability through synergistic attention mechanisms.}
\label{fig:cross_domain}
\end{figure*}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{plots/fig5_label_efficiency_enhanced.pdf}
\caption{Comprehensive Label Efficiency Analysis of PASE-Net: (a) Main performance comparison between zero-shot and fine-tuned approaches across label percentages, with error bars showing standard deviation and value labels for clarity; (b) Relative improvement ratio demonstrating up to 585\% performance gains through fine-tuning; (c) Learning curves with confidence regions showing smooth performance progression and convergence characteristics; (d) Performance gap analysis identifying the critical 20\% label threshold where gains plateau; (e) Label efficiency metric (F1/Label\%) revealing optimal cost-benefit at 10-20\% labels with diminishing returns beyond. The analysis validates that PASE-Net achieves 82.1\% macro-F1 with only 20\% labeled data (98.6\% of fully-supervised performance), enabling 80\% reduction in annotation costs for practical deployments.}
